...
import java.net.URL
import com.example.hutaburakari.DetailContent
...
class ThreadMonitorWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    ...
    private fun archiveMedia(context: Context, threadUrl: String, list: List<DetailContent>): List<DetailContent> {
        ...
        fun ensureDownloaded(remoteUrl: String): String? {
            ...
            return try {
                val req = Request.Builder().url(remoteUrl).header("User-Agent", com.example.hutaburakari.Ua.STRING).build()
                val resp = com.example.hutaburakari.NetworkModule.okHttpClient.newCall(req).execute()
                resp.use { r ->
