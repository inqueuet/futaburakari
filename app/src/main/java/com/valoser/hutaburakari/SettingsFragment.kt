    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        bgPref?.setOnPreferenceChangeListener { _, newValue ->
            ...
            requireContext().getSharedPreferences(
                com.valoser.hutaburakari.worker.ThreadMonitorWorker.PREFS_BG,
                android.content.Context.MODE_PRIVATE
            ).edit().putBoolean(
                com.valoser.hutaburakari.worker.ThreadMonitorWorker.KEY_BG_ENABLED,
                enabled
            ).apply()

            if (!enabled) {
                com.valoser.hutaburakari.worker.ThreadMonitorWorker.cancelAll(requireContext())
            } else {
